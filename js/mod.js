...
var displayThings = [
	function () { return `捐赠：https://ifdian.net/a/qwqe198` },
	function () { if (player.esc.points.gte(6) || (hasUpgrade("cq", 61) && inChallenge("cq", 13))) return `点数获取^b ,b=${format(layers.a.effect(), 5)}` },
function () { if (hasAchievement("rw", 81) ) return `上限${format(layers.grz.pthc())}` }
]
